...
public class CandyHolder : MonoBehaviour {
    ...
    void OnGUI ()
    {
        ...
        if (counter > 0) label = label + " (" + counter + "s)";     // 회복 카운트의 표시
    ...
    {
        // 사탕의 투척 기회가 디폴트보다 적고
        // 회복 카운트를 하고 있지 않을 때 카운트를 스타트시킨다
        if (candy < DefaultCandyAmount && counter <= 0)     // 회복 카운트 스타트의 판정
        {
            StartCoroutine(RecoverCandy());     // 코 루틴의 스타트
        }
    }
